...
extension DetailedCategoryTableViewCell: UICollectionViewDelegateFlowLayout, UICollectionViewDataSource{
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        switch index{
        case 0:
            return CGSize(width: collectionView.frame.size.width - 40 / 2, height: collectionView.frame.size.height/2) //self.getMostOrderedCollectionViewCell(indexPath: indexPath).frame.size
        case 1:
            return CGSize(width: collectionView.frame.size.width - 30 / 4, height: collectionView.frame.size.height/2 - 10)//self.getFavouriteFoodCollectionViewCell(indexPath: indexPath).frame.size
        case 2:
            return CGSize(width: collectionView.frame.size.width - 10, height: 250) //self.getDetailCollectionViewCell(indexPath: indexPath).frame.size
        default:
            break
        }
        return CGSize()
